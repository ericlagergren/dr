...
// NOTE(eric): storing the private and public halves of the key
// pair side-by-side and always requiring the full key pair
// reduces the likelihood that the wrong public key is used. H/T
// libsodium.

// KeyPair is a complete (private, public) key pair.
type KeyPair []byte
...
func (h Header) Append(buf []byte) []byte {
	n := len(buf)
	buf = append(buf, make([]byte, 8+len(h.PublicKey))...)
	binary.BigEndian.PutUint32(buf[n:n+4], uint32(h.PN))
	binary.BigEndian.PutUint32(buf[n+4:n+8], uint32(h.N))
	buf = append(buf[n+8:], h.PublicKey...)
	return buf
}
...
func (h *Header) Decode(data []byte) error {
	...
	h.PN = int(binary.BigEndian.Uint32(data[0:4]))
	h.N = int(binary.BigEndian.Uint32(data[4:8]))
	h.PublicKey = append(h.PublicKey[:0], data[8:]...)
	return nil
}
...
// The shared key SK must be negotiated with the peer ahead of
// time.
func NewRecv(r Ratchet, SK []byte, priv KeyPair, opts ...Option) (*Session, error) {
	s := &Session{
		r: r,
	}
	for _, fn := range opts {
